...
// Give KaTeX / fonts / animations / lazy-loaded images time to settle.
// 1800ms is empirically needed for the flagship cert cards (~440KB jpegs)
// to actually render in headless captureBeyondViewport mode.
...
if (fullPage) {
  // For full-page shots, scroll through the document first so the
  // IntersectionObserver fires on every [data-reveal] target. Without this,
  // sections below the initial viewport stay at opacity:0.
  const { result: h1 } = await sendSession('Runtime.evaluate', {
    expression: 'document.documentElement.scrollHeight',
    returnByValue: true,
  });
  const totalScroll = Number(h1.value) || height;
  for (let y = 0; y < totalScroll; y += Math.floor(height * 0.8)) {
    await sendSession('Runtime.evaluate', { expression: `window.scrollTo(0, ${y})` });
    await new Promise(r => setTimeout(r, 80));
  }
  await sendSession('Runtime.evaluate', { expression: 'window.scrollTo(0, 0)' });
  await new Promise(r => setTimeout(r, 200));
  const totalH = Math.min(totalScroll, 4000);
