// v6.3 Integrated Audit — 6 pages x 3 modes (desktop, mobile, full).
// Cloned from v63-integrated-shot.mjs, swaps port 8765 → 8767 and prefix v62-final → v63-.
// Verifies the v6.3 surface: EarthGlobe + MathBehindTape live on /.
...
async function snapOne(browser, name, kind, path) {
  ...
  const url = `${BASE}${path}`;
  const shotView = `${OUT}/v63-${name}-${kind}.png`;
  const shotFull = `${OUT}/v63-${name}-full.png`;
  ...
  try {
    ...
    status.ok = true;
    console.log('[v63-int] OK ', name, kind, status.http, `reqs=${requests.length} fails=${assetFails.length}`);
  } catch (e) {
    status.ok = false;
    status.err = e.message;
    console.log('[v63-int] FAIL', name, kind, status.err);
  } finally {
    await ctx.close();
    return status;
  }
...
(async () => {
  ...
  writeFileSync(`${OUT}/v63-integrated-raw.json`, JSON.stringify(results, null, 2));
  console.log('[v63-int] manifest written:', `${OUT}/v63-integrated-raw.json`);

  const fail = results.filter(r => !r.ok).length;
  console.log('[v63-int] DONE', { total: results.length, fail });
  process.exit(0);
