// v6.2 Integrated Audit — 6 pages x 3 modes (desktop, mobile, full).
// Cloned from v61-integrated-shot.mjs per the v61-patterns.md pattern,
// extended to 6 pages (adds /proof/) and the v6.2 component surface.
...
async function snapOne(browser, name, kind, path) {
  ...
  const url = `${BASE}${path}`;
  const shotView = `${OUT}/v62-final-${name}-${kind}.png`;
  const shotFull = `${OUT}/v62-final-${name}-full.png`;
  ...
  try {
    ...
    status.ok = true;
    console.log('[v62-int] OK ', name, kind, status.http, `reqs=${requests.length} fails=${assetFails.length}`);
  } catch (e) {
    status.ok = false;
    status.err = e.message;
    console.log('[v62-int] FAIL', name, kind, status.err);
  } finally {
    await ctx.close();
    return status;
  }
...
(async () => {
  ...
  writeFileSync(`${OUT}/v62-integrated-raw.json`, JSON.stringify(results, null, 2));
  console.log('[v62-int] manifest written:', `${OUT}/v62-integrated-raw.json`);

  const fail = results.filter(r => !r.ok).length;
  console.log('[v62-int] DONE', { total: results.length, fail });
  process.exit(0);
