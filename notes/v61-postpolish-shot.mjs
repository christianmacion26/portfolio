// v6.1 Post-Polish Re-Audit — re-shoot 5 pages × 2 viewports = 10 shots
// Per Quality_Officer directive: re-run the same 7 visual checks but with the
// new filename convention `v61-postpolish-*.png`. Verifies P0/P1/P2 fixes
// closed the v6.1 integrated audit gaps.
...
async function snapOne(browser, name, kind, path) {
  ...
  const url = `${BASE}${path}`;
  const shotView = `${OUT}/v61-postpolish-${name}-${kind}.png`;
  ...
  try {
    ...
    status.ok = true;
    console.log('[v61-pp]', 'OK', name, kind, status.http, `reqs=${requests.length} fails=${assetFails.length}`);
  } catch (e) {
    status.ok = false;
    status.err = e.message;
    console.log('[v61-pp]', 'FAIL', name, kind, status.err);
  } finally {
    await ctx.close();
    return status;
  }
...
(async () => {
  ...
  writeFileSync(`${OUT}/v61-postpolish-raw.json`, JSON.stringify(results, null, 2));
  console.log('[v61-pp] manifest written');
  const fail = results.filter(r => !r.ok).length;
  console.log('[v61-pp] DONE', { total: results.length, fail });
  process.exit(0);
